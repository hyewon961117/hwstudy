...
ui <- fluidPage(
  fluidRow(column(9, selectModUI("selectmap"),div(style="height:45px")),
           column(3, sliderInput("range_time", "Construction Year", 
                                 sep="", min=1960, max=2021, 
                                 value=c(1970,2020)),
                  sliderInput("range_area","Area",
                              sep="", min=0, max=350, 
                              value=c(0,200)), )),
  tabsetPanel(
    tabPanel("Chart",
...
server <- function(input, output, session){
  ...
  output$regression <- renderPlot({
    ...
    grob_1 <- grobTree(textGrob(paste0("All: ",coef_all), x=0.05, y=0.84, hjust=0, gp=gpar(col="blue", fontsize=13)))
    grob_2 <- grobTree(textGrob(paste0("Sel: ",coef_sel), x=0.05, y=0.95, hjust=0, gp=gpar(col="red", fontsize=16, fontface="bold")))
    gg <- ggplot(sel, aes(x=Group.1, y=x, group=1)) + 
      geom_smooth(color="red", size=1.5, se=F)+xlab("Year")+ylab("Price")+theme(axis.text.x=element_text(angle=90))+
      stat_smooth(method="lm", linetype = "dashed", se=F) + theme_bw()
    gg + geom_smooth(data=all, aes(x=Group.1, y=x, group=1, se=F), color="blue", size=1, se=F) + 
